const XCU = artifacts.require('CuprumXCU.sol');
...
// Test that XCU operates correctly as an Ownable token.
contract('Ownable XCU', function ([_, admin, anotherAccount, anotherAccount2, owner]) {
  beforeEach(async function () {
    const xcu = await XCU.new({from: owner});
  ...
  describe('as an initializable token', function () {
    ...
    it('constructor initializes the implementation contract and pauses it to avoid misleading state there', async function () {
      const isPaused = await this.xcu.paused();
      assert.strictEqual(isPaused, true);
      const currentOwner = this.xcu.owner();
      assert.notStrictEqual(currentOwner, ZERO_ADDRESS);
      await assertRevert(this.xcu.initialize());
    });
  });
});
